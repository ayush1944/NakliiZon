...
function AdminEditProduct({
  ...
  // Initialize selectedImages with already uploaded images from productData
  const [selectedImages, setSelectedImages] = useState(
    Array.isArray(productData?.productImage) ? productData.productImage.map(img => img.url || img) : []
  );
              ...
              {selectedImages.length > 0 ? (
                selectedImages.map((image, index) => (
                  <div key={index} className="relative">
                    <img
                      src={typeof image === 'string' ? image : image.url}  // Handle both local and uploaded images
                      alt={`Selected ${index}`}
